...
/// permutation map is not a minor identy, a dimension is potentially out of
/// bounds, or if it would merge two dimensions that are both transfer
/// dimensions.
...
LogicalResult TransferOpOfExpandShapeOpFolder::matchAndRewrite(
  ...
  SmallVector<ReassociationIndices> reassocs = expand.getReassociationIndices();
  if (!hasTrivialReassociationSuffix(reassocs, vecRank - 1))
    return rewriter.notifyMatchFailure(
        op, "expand_shape folding would merge two transfer dimensions");
