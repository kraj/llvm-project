...
static bool simplifyAffineMinMaxOp(RewriterBase &rewriter, AffineOp affineOp) {
  ...
  SmallVector<AffineMap> variableMaps =
      llvm::map_to_vector(variables, [](Variable v) { return v.getMap(); });
  LLVM_DEBUG({
    DBGS() << "- constructed variables are: "
           << llvm::interleaved_array(variableMaps) << "`\n";
  });
  ...
  LLVM_DEBUG({ DBGS() << "- starting from map: " << affineMap << "\n"; });
  LLVM_DEBUG({ DBGS() << "- creating new map with: \n"; });
  LLVM_DEBUG({ DBGS() << "--- dims: " << affineMap.getNumDims() << "\n"; });
  LLVM_DEBUG({ DBGS() << "--- syms: " << affineMap.getNumSymbols() << "\n"; });
  LLVM_DEBUG(
...
mlir::affine::createSimplifyAffineMinMaxPass() {
  return std::make_unique<SimplifyAffineMinMaxPass>();
}
...
void SimplifyAffineMinMaxPass::runOnOperation() {
  ...
  patterns.add<SimplifyAffineMaxOp, SimplifyAffineMinOp, SimplifyAffineApplyOp>(
      func.getContext());
  FrozenRewritePatternSet frozenPatterns(std::move(patterns));
  applyPatternsGreedily(func, std::move(frozenPatterns));
}
