...
public:
  enum class Kind : uint8_t {
    ...
    /// loan set.
    OriginFlow,
    /// Loans held by the source origin are projected (their access paths are extended
    /// by a path element) and then flowed into the destination origin.
    /// Example: if `obj` holds loan `{x}`, `p = obj.field` projects `{x}` with `field`
    /// to `{x.field}` and flows it to `p`.
    Projection,
