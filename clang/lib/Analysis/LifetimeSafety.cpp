...
public:
  ...
  void checkUse(const UseFact *UF) {
    ...
    for (LoanID BadLoan : DefaultedLoans) {

      if (FinalWarningsMap.count(BadLoan) &&
          CurrentConfidence <= FinalWarningsMap[BadLoan].Level)
        continue;

      const Loan &L = FactMgr.getLoanMgr().getLoan(BadLoan);
      auto *EF = AllExpiredLoans.lookup(BadLoan);
      ...
      FinalWarningsMap[BadLoan] = {IssueExpr, ExpiryLoc, UF->getUseExpr(),
                                   CurrentConfidence};
    }
  }
