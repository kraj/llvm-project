...
// CHECK: CXXMethodDecl {{.*}} Load 'vector<element_type (vector<int, 3>), element_count>'
...
// CHECK: CXXMethodDecl {{.*}} Load 'vector<element_type (vector<int, 3>, vector<int, 2>), element_count>'
// CHECK-NEXT: ParmVarDecl {{.*}} Location 'vector<int, 3>'
...
// CHECK: CXXMethodDecl {{.*}} operator[] 'vector<element_type const hlsl_device &(vector<unsigned int, 2>) const, element_count>' inline
// CHECK-NEXT: ParmVarDecl {{.*}} Index 'vector<unsigned int, 2>'
// CHECK-NEXT: CompoundStmt
// CHECK-NEXT: ReturnStmt
// CHECK-NEXT: UnaryOperator {{.*}} 'vector<element_type hlsl_device, element_count>' lvalue prefix '*' cannot overflow
// CHECK-NEXT: CStyleCastExpr {{.*}} 'vector<element_type hlsl_device *, element_count>' <Dependent>
// CHECK-NEXT: CallExpr {{.*}} '<dependent type>'
