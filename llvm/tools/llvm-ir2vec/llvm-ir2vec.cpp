  ...
  unsigned generateTriplets(const Function &F, raw_ostream &OS) const {
    ...
    for (const BasicBlock &BB : F) {
      for (const auto &I : BB.instructionsWithoutDebug()) {
        unsigned Opcode = Vocabulary::getSlotIdx(I.getOpcode());
        unsigned TypeID = Vocabulary::getSlotIdx(I.getType()->getTypeID());
        ...
        for (const Use &U : I.operands()) {
          unsigned OperandID = Vocabulary::getSlotIdx(U.get());
