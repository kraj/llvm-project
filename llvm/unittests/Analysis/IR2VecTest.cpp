...
#undef EXPECT_TYPE_SLOT
  ...
  // Test getSlotIdx for Value operands
  LLVMContext Ctx;
  Module M("TestM", Ctx);
  FunctionType *FTy =
...
#define EXPECTED_VOCAB_OPERAND_SLOT(X)                                         \
  ...
  EXPECT_EQ(Vocabulary::getSlotIdx(F),
  ...
  EXPECT_EQ(Vocabulary::getSlotIdx(C),
...
TEST(IR2VecVocabularyTest, NumericIDMapInvalidInputs) {
  ...
  EXPECT_DEATH(Vocabulary::getSlotIdx(0u), "Invalid opcode");
  EXPECT_DEATH(Vocabulary::getSlotIdx(MaxOpcodes + 1), "Invalid opcode");

  // Test invalid type IDs
  EXPECT_DEATH(Vocabulary::getSlotIdx(static_cast<Type::TypeID>(MaxTypeIDs)),
               "Invalid type ID");
  EXPECT_DEATH(
      Vocabulary::getSlotIdx(static_cast<Type::TypeID>(MaxTypeIDs + 10)),
      "Invalid type ID");
}
#endif // NDEBUG
#endif // GTEST_HAS_DEATH_TEST
...
#define EXPECT_OPCODE(NUM, OPCODE, CLASS)                                      \
  EXPECT_EQ(Vocabulary::getStringKey(Vocabulary::getSlotIdx(NUM)),             \
...
#define EXPECT_TYPE_TO_CANONICAL(TypeIDTok, CanonEnum, CanonStr)               \
  EXPECT_EQ(Vocabulary::getStringKey(Vocabulary::getSlotIdx(Type::TypeIDTok)), \
            CanonStr);
