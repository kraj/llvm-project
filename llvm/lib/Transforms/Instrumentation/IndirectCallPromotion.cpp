...
private:
  ...
  Instruction *getVTableInfos(const CallBase *CB,
                              VTableGUIDCountsMap &VTableGUIDCounts,
...
Instruction *IndirectCallPromoter::getVTableInfos(
  ...
  Instruction *VPtr = nullptr;
  const auto &VirtualCallInfo = Iter->second;
  VPtr = VirtualCallInfo.VPtr;
...
bool IndirectCallPromoter::processFunction(ProfileSummaryInfo *PSI) {
  ...
  for (auto *CB : findIndirectCalls(F)) {
    ...
    VTableGUIDCountsMap VTableGUIDCounts;
    Instruction *VPtr =
        getVTableInfos(CB, VTableGUIDCounts, PromotionCandidates);
...
bool IndirectCallPromoter::isProfitableToCompareVTables(
  ...
  // FIXME: Do not compare vtables if vtable load sequence cannot sink to
  // indirect fall through.
