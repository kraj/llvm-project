...
void OpenMPIRBuilder::emitReductionListCopy(
  ...
  for (auto En : enumerate(ReductionInfos)) {
    ...
    case CopyAction::RemoteLaneToThread: {
      ...
      Builder.restoreIP(AllocaIP);
      Type *DestAllocaType = IsByRefElem ? RI.ByRefElementType : RI.ElementType;
    ...
    if (ShuffleInElement) {
      ...
      if (IsByRefElem) {
        ShuffleType = Builder.getFloatTy();

        ShuffleSrcAddr = Builder.CreateGEP(RI.ByRefElementType, ShuffleSrcAddr,
                                           {Zero, Zero});
      ...
      if (IsByRefElem) {
        auto *GEP =
            Builder.CreateGEP(RI.ByRefElementType,
...
Expected<Function *> OpenMPIRBuilder::emitInterWarpCopyFunction(
  ...
  for (auto En : enumerate(ReductionInfos)) {
    ...
    unsigned RealTySize = M.getDataLayout().getTypeAllocSize(
        IsByRef[En.index()] ? Builder.getFloatTy() : RI.ElementType);
    for (unsigned TySize = 4; TySize > 0 && RealTySize > 0; TySize /= 2) {
      ...
      if (IsByRef[En.index()]) {
        Type *Int32Ty = Builder.getInt32Ty();
        Constant *Zero = ConstantInt::get(Int32Ty, 0);
        ElemPtr = Builder.CreateGEP(RI.ByRefElementType, ElemPtr, {Zero, Zero});
      ...
      if (IsByRef[En.index()]) {
        ...
        Constant *Zero = ConstantInt::get(Int32Ty, 0);
        TargetElemPtr =
            Builder.CreateGEP(RI.ByRefElementType, TargetElemPtr, {Zero, Zero});
