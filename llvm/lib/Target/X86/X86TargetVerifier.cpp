...
// verifier.
//
// The checks here are the ones that are *subtarget/feature-dependent* and so
// cannot be expressed by the generic, triple-only IR verifier: they need to
// know the specific CPU/features selected via the target-cpu/target-features
// function attributes. The MCSubtargetInfo is derived from those attributes, so
// no TargetMachine is required and the verifier can run from generic pipelines.
//
//===----------------------------------------------------------------------===//
...
private:
  ...
  /// Build an MCSubtargetInfo from \p F's target-cpu / target-features.
  std::unique_ptr<MCSubtargetInfo> getSubtargetInfo(const Function &F) const;
};

/// \returns true if \p I produces or consumes an x86_amx-typed value.
...
X86TargetVerify::getSubtargetInfo(const Function &F) const {
  std::string Error;
...
void X86TargetVerify::verifyFunctionChecks(Function &F,
  ...
  for (const Instruction &I : instructions(F)) {
    // (A) An instruction-set intrinsic requires its feature on the selected
    // subtarget.
    if (const auto *CB = dyn_cast<CallBase>(&I)) {
      if (const Function *CF = CB->getCalledFunction()) {
        ...
        // (A) The 128/256-bit (EVEX.128/256) forms of an AVX-512 intrinsic
        // additionally require AVX512VL, which AVX512F alone does not provide
        // (e.g. Knights Landing has AVX512F but not VL). The width is encoded
        // as a ".128"/".256" suffix on the intrinsic name.
    ...
    // (A) The x86_amx type requires the AMX-TILE feature on the selected
    // subtarget.
    if (usesAMXType(I))
      Check(HasAMXTILE,
...
bool X86TargetVerify::run(Function &F) {
  ...
  std::unique_ptr<MCSubtargetInfo> STI = getSubtargetInfo(F);
  if (!STI)
    return IsValid;
...
namespace llvm {
std::unique_ptr<TargetVerify> createX86TargetVerify(Module &M) {
  return std::make_unique<X86TargetVerify>(&M);
}
} // namespace llvm
