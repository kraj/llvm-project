...
SDValue SITargetLowering::lowerPointerAsRsrcIntrin(SDNode *Op,
  ...
  if (Subtarget->has45BitNumRecordsBufferResource()) {
    ...
    if (std::optional<uint32_t> ConstStride = GetConstStride()) {
      ShiftedStride = DAG.getConstant(
          *ConstStride ? static_cast<uint64_t>(*ConstStride) << 44 : 0, Loc,
          MVT::i64);
    } else {
    ...
    std::optional<uint32_t> ConstStride = GetConstStride();
