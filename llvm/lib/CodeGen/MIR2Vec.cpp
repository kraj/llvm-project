...
Expected<MIRVocabulary> MIRVocabulary::createDummyVocabForTest(
  ...
  MIRVocabulary TempVocab({}, {}, {}, {}, TII, TRI, MRI);
  ...
  for (const auto &COpcodeName : TempVocab.UniqueBaseOpcodeNames) {
    DummyOpcMap[COpcodeName] = Embedding(Dim, DummyVal);
    DummyVal += 0.1f;
  }

  // Create dummy embeddings for all common operand types
  for (const auto &CommonOperandName : CommonOperandNames) {
    DummyOperandMap[CommonOperandName.str()] = Embedding(Dim, DummyVal);
    DummyVal += 0.1f;
  }

  // Create dummy embeddings for all register operand types
  for (const auto &RegOperandName : TempVocab.RegisterOperandNames) {
    DummyPhyRegMap[RegOperandName] = Embedding(Dim, DummyVal);
    DummyVirtRegMap[RegOperandName] = Embedding(Dim, DummyVal);
    DummyVal += 0.1f;
  }
