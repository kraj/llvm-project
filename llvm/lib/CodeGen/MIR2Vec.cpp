...
bool Vocabulary::isValid() const {
  return UniqueBaseOpcodeNames.size() > 0 && Storage.isValid();
}
...
const Embedding &Vocabulary::operator[](unsigned Index) const {
...
std::string Vocabulary::extractBaseOpcodeName(StringRef InstrName) {
...
unsigned Vocabulary::getCanonicalIndexForBaseName(StringRef BaseName) const {
...
std::string Vocabulary::getStringKey(unsigned Pos) const {
  assert(isValid() && "MIR2Vec Vocabulary is invalid");
...
void Vocabulary::generateStorage(const VocabMap &OpcodeMap,
...
void Vocabulary::buildCanonicalOpcodeMapping(const TargetInstrInfo &TII) {
...
//===----------------------------------------------------------------------===//
// MIR2VecVocabAnalysis Implementation
//===----------------------------------------------------------------------===//

char MIR2VecVocabAnalysis::ID = 0;
INITIALIZE_PASS_BEGIN(MIR2VecVocabAnalysis, "mir2vec-vocab",
                      "MIR2Vec Vocabulary Analysis", false, true)
INITIALIZE_PASS_DEPENDENCY(MachineModuleInfoWrapperPass)
INITIALIZE_PASS_END(MIR2VecVocabAnalysis, "mir2vec-vocab",
                    "MIR2Vec Vocabulary Analysis", false, true)

StringRef MIR2VecVocabAnalysis::getPassName() const {
  return "MIR2Vec Vocabulary Analysis";
}

Error MIR2VecVocabAnalysis::readVocabulary() {
...
void MIR2VecVocabAnalysis::emitError(Error Err, LLVMContext &Ctx) {
  Ctx.emitError(toString(std::move(Err)));
}

mir2vec::Vocabulary
MIR2VecVocabAnalysis::getMIR2VecVocabulary(const Module &M) {
  if (StrVocabMap.empty()) {
    if (Error Err = readVocabulary()) {
      emitError(std::move(Err), M.getContext());
      return mir2vec::Vocabulary(std::move(StrVocabMap), nullptr);
    }
  }
  ...
  for (const auto &F : M) {
    ...
    if (auto *MF = MMI.getMachineFunction(F)) {
      ...
      return mir2vec::Vocabulary(std::move(StrVocabMap), TII);
    }
  }
...
char MIR2VecVocabPrinterPass::ID = 0;
INITIALIZE_PASS_BEGIN(MIR2VecVocabPrinterPass, "print-mir2vec-vocab",
                      "MIR2Vec Vocabulary Printer Pass", false, true)
INITIALIZE_PASS_DEPENDENCY(MIR2VecVocabAnalysis)
INITIALIZE_PASS_DEPENDENCY(MachineModuleInfoWrapperPass)
INITIALIZE_PASS_END(MIR2VecVocabPrinterPass, "print-mir2vec-vocab",
                    "MIR2Vec Vocabulary Printer Pass", false, true)

bool MIR2VecVocabPrinterPass::runOnMachineFunction(MachineFunction &MF) {
  return false;
}

bool MIR2VecVocabPrinterPass::doFinalization(Module &M) {
  auto &Analysis = getAnalysis<MIR2VecVocabAnalysis>();
...
MachineFunctionPass *llvm::createMIR2VecVocabPrinterPass(raw_ostream &OS) {
  return new MIR2VecVocabPrinterPass(OS);
}
