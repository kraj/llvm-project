...
void CovFunV7::encode(raw_ostream &OS) const {
...
std::vector<std::string> CovMapV7::generateAccFilenames(
...
void CovMapV7::regenerateFilenames(
...
CovMapV7::encodeFilenames(const std::optional<ArrayRef<StringRef>> &AccFilesOpt,
                          bool Compress) const {
...
Expected<uint64_t> CovFunV7::decode(CovMapByRefTy &CovMapByRef,
...
void CovMapV7::encode(raw_ostream &OS) const {
...
Expected<uint64_t> CovMapV7::decode(const ArrayRef<uint8_t> Content,
                                    uint64_t Offset, const DecoderParam &Param,
                                    bool IsLE) {
  DecoderContext Data(Content, Param, IsLE);
  Data.seek(Offset);
...
void CovFunV7::mapping(llvm::yaml::IO &IO) {
...
void CovMapV7::mapping(llvm::yaml::IO &IO) {
  IO.mapRequired("FilenamesRef", FilenamesRef);
...
struct CovFunSection : ELFYAML::CovMapSectionBase {
  ...
  static Expected<std::vector<CovFunV7>> decode(CovMapByRefTy &CovMapByRef,
                                                InstrProfSymtab *Symtab,
...
class CovMapFilenamesResolver {
  ...
  std::vector<CovFunV7 *> UnresolvedCovFuns;

protected:
  CovMapByRefTy CovMapByRef;
  std::vector<CovMapV7> TempCovMaps; // For Decoder

public:
  void collectCovMap(std::vector<CovMapV7> &CovMaps) {
    for (auto &CovMap : CovMaps)
      CovMapByRef[CovMap.FilenamesRef] = &CovMap;
  }

  void moveAndCollectCovMap(std::vector<CovMapV7> &&CovMaps) {
    TempCovMaps = std::move(CovMaps);
    collectCovMap(TempCovMaps);
  }

  void collectCovFunFilenames(std::vector<CovFunV7> &CovFuns) {
  ...
  void decMayeResetFilenames(std::vector<CovMapV7> &CovMaps) {
    for (auto &CovMap : CovMaps) {
...
class DecoderImpl : public Decoder, CovMapFilenamesResolver {
  ...
  DenseMap<uint64_t, std::vector<CovFunV7>> TempCovFuns;
