...
; RUN: llc < %s -mtriple arm64e-apple-darwin             -print-isel-input -stop-after=finalize-isel -global-isel=0                      2>&1 | FileCheck --check-prefixes=DAGISEL %s
; RUN: llc < %s -mtriple arm64e-apple-darwin             -print-isel-input -stop-after=finalize-isel -global-isel=1 -global-isel-abort=1 2>&1 | FileCheck --check-prefixes=GISEL   %s
; RUN: llc < %s -mtriple aarch64-linux-gnu -mattr=+pauth -print-isel-input -stop-after=finalize-isel -global-isel=0                      2>&1 | FileCheck --check-prefixes=DAGISEL %s
; RUN: llc < %s -mtriple aarch64-linux-gnu -mattr=+pauth -print-isel-input -stop-after=finalize-isel -global-isel=1 -global-isel-abort=1 2>&1 | FileCheck --check-prefixes=GISEL   %s
