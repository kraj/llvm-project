...
//
// RUN: %clangxx -O0 -g -mllvm -enable-instrumentor -mllvm -instrumentor-read-config-files=%S/../../lib/instrumentor-examples/pointer-tracking/pointer_tracking_config.json %s -L%pointer_tracking_lib_dir -l%pointer_tracking_lib -o %t
// RUN: %t | FileCheck %s
//
