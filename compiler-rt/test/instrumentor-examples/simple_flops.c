...
//
// RUN: %clang -O0 -g -mllvm -enable-instrumentor -mllvm -instrumentor-read-config-files=%S/../../lib/instrumentor-examples/flop-counter/flop_counter_config.json %s -L%flop_counter_lib_dir -l%flop_counter_lib -o %t
// RUN: %t | FileCheck %s
//
