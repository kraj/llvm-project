...
def _object_linker_input(ctx, object_file):
    return cc_common.create_linker_input(
        owner = ctx.label,
        user_link_flags = depset([object_file.path]),
        additional_inputs = depset([object_file]),
    )
...
def _extract_object_file_impl(ctx):
    ...
    return [
        ...
        # Package CcInfo so this rule be may be used in a cc_library's deps.
        CcInfo(
            linking_context = cc_common.create_linking_context(
                linker_inputs = depset([_object_linker_input(ctx, output)]),
            ),
        ),
    ]
...
merge_relocatable_object = rule(
    ...
    doc = """Merges multiple C++ object files into a single relocatable object file (.o).

    This rule mimics CMake's `merge_relocatable_object` (which runs `clang -r` or equivalent).
    It extracts the compilation outputs (.o files) of the direct dependencies in `deps` and
    runs the toolchain's linker driver with `-r -nostdlib` to combine them.

    The output target provides `CcInfo` containing the merged object file as a linker input.
    """,
    attrs = {
