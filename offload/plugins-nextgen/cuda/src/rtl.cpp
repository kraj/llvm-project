...
struct CUDAKernelTy : public GenericKernelTy {
  ...
  Error initImpl(GenericDeviceTy &GenericDevice,
    ...
    int SharedMemSize;
    Res = cuFuncGetAttribute(&SharedMemSize, CU_FUNC_ATTRIBUTE_SHARED_SIZE_BYTES, Func);
    if (auto Err = Plugin::check(Res, "Error in cuFuncGetAttribute: %s"))
      return Err;
...
struct CUDADeviceTy : public GenericDeviceTy {
  ...
  Error initImpl(GenericPluginTy &Plugin) override {
	...
	if (auto Err = getDeviceAttr(CU_DEVICE_ATTRIBUTE_MAX_SHARED_MEMORY_PER_BLOCK, MaxSharedMem))
      return Err;
