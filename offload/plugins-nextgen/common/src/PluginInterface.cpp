...
Error GenericKernelTy::launch(GenericDeviceTy &GenericDevice, void **ArgPtrs,
  ...
  if (StaticBlockMemSize > MaxBlockMemSize)
    return Plugin::error(ErrorCode::INVALID_ARGUMENT, "Static block memory size exceeds maximum");
  else if (!KernelArgs.Flags.AllowDynCGroupMemFallback && TotalBlockMemSize > MaxBlockMemSize)
    return Plugin::error(ErrorCode::INVALID_ARGUMENT, "Static and dynamic block memory size exceeds maximum");

  void *FallbackBlockMem = nullptr;
  if (DynBlockMemSize && (!GenericDevice.hasNativeBlockSharedMem() || TotalBlockMemSize > MaxBlockMemSize)) {
    auto AllocOrErr = GenericDevice.dataAlloc(NumBlocks[0] * DynBlockMemSize,
                                            /*HostPtr=*/nullptr,
                                            TargetAllocTy::TARGET_ALLOC_DEVICE);
    if (!AllocOrErr)
      return AllocOrErr.takeError();
...
int64_t GenericPluginTy::query_device_info(int32_t DeviceId, DeviceQueryKind Query) {
