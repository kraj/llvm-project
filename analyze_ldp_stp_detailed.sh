...
analyze_changes() {
    ...
    # Count different instruction types (ensure we get clean numeric output)
    local orig_ldp=$(grep -c -E '\bldp\b' "$orig_file" 2>/dev/null | tr -d '[:space:]' || echo "0")
    local orig_stp=$(grep -c -E '\bstp\b' "$orig_file" 2>/dev/null | tr -d '[:space:]' || echo "0")
    local orig_ldr=$(grep -c -E '\bldr\b' "$orig_file" 2>/dev/null | tr -d '[:space:]' || echo "0")
    local orig_str=$(grep -c -E '\bstr\b' "$orig_file" 2>/dev/null | tr -d '[:space:]' || echo "0")

    local patched_ldp=$(grep -c -E '\bldp\b' "$patched_file" 2>/dev/null | tr -d '[:space:]' || echo "0")
    local patched_stp=$(grep -c -E '\bstp\b' "$patched_file" 2>/dev/null | tr -d '[:space:]' || echo "0")
    local patched_ldr=$(grep -c -E '\bldr\b' "$patched_file" 2>/dev/null | tr -d '[:space:]' || echo "0")
    local patched_str=$(grep -c -E '\bstr\b' "$patched_file" 2>/dev/null | tr -d '[:space:]' || echo "0")
    ...
    orig_ldr=${orig_ldr:-0}
    orig_str=${orig_str:-0}
    patched_ldp=${patched_ldp:-0}
    patched_stp=${patched_stp:-0}
    patched_ldr=${patched_ldr:-0}
    patched_str=${patched_str:-0}
...
total_files=$(wc -l < "$FILES_LIST" | tr -d '[:space:]')
processed_files=0
found_regressions=0
...
while IFS= read -r orig_file; do
    ...
    if [ -f "$patched_file" ]; then
        # Count ldp and stp instructions in both files (ensure clean numeric output)
        orig_count=$(grep -c -E '\b(ldp|stp)\b' "$orig_file" 2>/dev/null | tr -d '[:space:]' || echo "0")
        patched_count=$(grep -c -E '\b(ldp|stp)\b' "$patched_file" 2>/dev/null | tr -d '[:space:]' || echo "0")

        # Ensure variables are numeric
        orig_count=${orig_count:-0}
        patched_count=${patched_count:-0}
