...
// Defined below, next to the loop-local category (c) predicate it drives.
...
public:
  ...
  void dumpEvaluation(llvm::raw_ostream &outputStream,
    ...
    // on the outside but has unstructured internals, so it still lowers to a
    // structured op whose body holds raw control flow -- without a distinct
    // marker that case is indistinguishable from a fully structured one.
...
/// Category (c): a loop that can be lowered structurally even though its body
/// holds unstructured control flow, because that control flow is confined to
/// the body and can be folded into an scf.execute_region.
///
      ...
      [&](Fortran::lower::pft::EvaluationList &list) {
        for (Fortran::lower::pft::Evaluation &e : list) {
          ...
          if (e.isA<parser::DoConstruct>() &&
            ...
            // Unstructured unnecessary, so assign rather than markControlFlow,
            // which only ever strengthens.
            e.controlFlow = Fortran::lower::pft::Evaluation::ControlFlow::
