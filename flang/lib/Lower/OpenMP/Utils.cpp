...
// Lower the stable base entity for an iterator map/motion locator. MapInfoOp
// keeps this base as var_ptr and represents the iterator-selected elements or
// sections with map bounds. For component array locators such as v%a(i) or
// v%a(i:i+1), return v%a so each iteration maps offsets from the same base.
...
getIteratorMapEntity(Fortran::lower::AbstractConverter &converter,
  ...
  assert(arrayBase);
  if (auto comp =
          Fortran::lower::detail::getRef<evaluate::Component>(*arrayBase)) {
    fir::ExtendedValue dataExv;
    if (semantics::IsAllocatableOrPointer(comp->symbol()))
